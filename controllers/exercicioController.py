...
def exercicioController():
    ...
    if request.method == 'POST':
        try:
            ...
            exercicio = Exercicio(nome=data['nome'],descricao=data['descricao'],repeticoes=data['repeticoes'],serie=data['serie'],codtreino=data['codtreino'],video=data['video'])
        ...
        try:
            ...
            put_exercicio.video      = data.get('video', put_exercicio.video)
            db.session.commit()
            return jsonify({'message': 'Exercício alterado com sucesso'}), 200
        except Exception as e:
